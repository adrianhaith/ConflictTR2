...
%
%
subjname = {'S01','S02','S03','S04'};%,'S106','S107','S108'};
...
for i = 1:length(subjname);
    ...
    data{i,4} = loadTRsubjData(subjname{i},{'arrow_spat_ForcedRT_1','arrow_spat_ForcedRT_2','arrow_spat_ForcedRT_3'});%,'col_all_ForcedRT_4','col_all_ForcedRT_5','col_all_ForcedRT_6'});
    data{i,5} = loadTRsubjData(subjname{i},{'col_arrow_ForcedRT_1','col_arrow_ForcedRT_2','col_arrow_ForcedRT_3'});
    data{i,6} = loadTRsubjData(subjname{i},{'col_spat_ForcedRT_1','col_spat_ForcedRT_2','col_spat_ForcedRT_3'});
...
save ConflictDataRaw_Expt4
%}
%%
clear all
load ConflictDataRaw_Expt4
%%
Nc = 6;
...
for s=1:length(subjname)
    for c=[4 5 6]; % conflict blocks
        switch c
            ...
            case 5 
                icongruent = find(d{c}.goalAng_col(s,:) == d{c}.goalAng_symb(s,:));
                iincongruent = find(d{c}.goalAng_col(s,:) ~= d{c}.goalAng_symb(s,:));
                
            case 6
                icongruent = find(d{c}.goalAng_spat(s,:) == d{c}.goalAng_col(s,:));
                iincongruent = find(d{c}.goalAng_spat(s,:) ~= d{c}.goalAng_col(s,:));
        end
        %icongruent = find(d{c}.goalAng_spat(s,:) == d{c}.goalAng_symb(s,:) & d{c}.goalAng_spat(s,:) == d{c}.goalAng_col(s,:));
        if(length(icongruent)>48)
            icongruent = icongruent(1:48);
        end
...
save ConflictDataRawCompact_Expt4 d
%}