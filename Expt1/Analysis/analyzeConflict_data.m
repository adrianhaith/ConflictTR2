...
data{1} = loadTRsubjData(subjname,{'spat_ForcedRT_1','spat_ForcedRT_2'});
data{2} = loadTRsubjData(subjname,{'symb_ForcedRT_1','symb_ForcedRT_2'});
data{3} = loadTRsubjData(subjname,{'conf_ForcedRT_1','conf_ForcedRT_2'});
...
for c=1:3
    ...
    title(condStr{c})
    plot(data{c}.RT,data{c}.reachDir_spat,'.')
    plot([0 RTmax],[30 30],'color',.7*[1 1 1],'linewidth',2);
    plot([0 RTmax],[-30 -30],'color',.7*[1 1 1],'linewidth',2);
    axis([0 RTmax -180 180])
    axis([0 RTmax -180 180])
...
for c = 1:3
    ...
    hit_symb = abs(data{c}.reachDir_spat)<TOL;
