...
for s = 1:length(subjname);
    ...
    data{s,2} = loadTRsubjData(subjname{s},{'arrow_forcedRT_1','arrow_forcedRT_2'});
...
for s=1:length(subjname)
    ...
    for i=1:4
        ii = find(data{s,2}.tFile(:,11)==i); 
        symb_key(s,i+1) = data{s,2}.tFile(ii(1),14);
    end
    % apply to conflict block
    for i=[1 3 5] % no arrow blocks
        data{s,i}.goalDir_symb = NaN*ones(data{s,i}.Ntrials,1)';
    end
    for i=[2 4 6 7 8 9] % arrow blocks
        data{s,i}.goalDir_symb = symb_key(s,data{s,i}.tFile(:,11)+1);
    end
...
for s=1:1
    ...
    subplot(3,1,2); hold on
    plot(RTplot,phit_spat(s,:,5),'k','linewidth',2)
    ...
    subplot(3,1,3); hold on
    plot(RTplot,phit_symb(s,:,6),'b','linewidth',2)
    plot(RTplot,phit_goal(s,:,6),'m','linewidth',2)
    plot([0 .7],[.25 .25],'k')
    axis([0 RTmax -.05 1.05])
    legend('arrow','color')
    legend boxoff
